...
static int stat_em_solid, stat_em_pass, in_pass;
                         ...
                         int *out_lo, int *out_hi)
{
    ...
    const float xa = cx + oa * cur_cam->focal / da;
    const float xb = cx + ob * cur_cam->focal / db;

    int lo = (int)floorf(xa < xb ? xa : xb);
    int hi = (int)ceilf (xa > xb ? xa : xb) - 1;
    if (lo < 0) lo = 0;
                        ...
                        float xs_a, float xs_b)
{
    ...
    for (int x = x1; x <= x2; x++) {
        ...
        if (t > clip_top[x]) clip_top[x] = (uint8_t)t;
        if (b < clip_bot[x]) clip_bot[x] = (uint8_t)b;
    }
    clip_blk_refresh(x1, x2);
}
...
{
    if (x2 < x1) return;
    if (in_pass) stat_em_pass++; else stat_em_solid++;
...
{
    const r_wall_t saved = cur_wall;
    cur_wall.tex = NULL;      /* emit_range becomes a no-op via r_draw_wall */
    clip_solid(first, last);
    clip_close(first, last);
    cur_wall = saved;
}
...
static void clip_pass(int first, int last)
{
    ...
    if (first < start->first) {
        if (last < start->first - 1) { emit_range(first, last); in_pass = 0; return; }
        emit_range(first, start->first - 1);
    }

    if (last <= start->last) { in_pass = 0; return; }
...
static void render_flats_inner(const subsector_t *ss)
{
    ...
    (void)ss; return;
#else
    if (r_ssdata[ss - subsectors].numpts < 3 || !ss->sector) return;
    ...
    const r_ssdata_t *sd = &r_ssdata[ss - subsectors];
    ...
    if (!span_of_bbox((float)rg->bx0, (float)rg->by0,
                      (float)rg->bx1, (float)rg->by1, &lo, &hi))
        { FDUMP("spanrej", 0, 0); r_drop_span++; return; }
    if (range_covered(lo, hi)) { FDUMP("covered", lo, hi); r_drop_cover++; stat_flatcull++; return; }
    ...
    const float light = (float)(sec->lightlevel < 0 ? 0 :
...
static void render_subsector(int num)
{
    ...
    if (ss->sector->validcount != r_thing_vc) {
        ...
        const float lit = (float)(ss->sector->lightlevel < 0 ? 0 :
                                  ss->sector->lightlevel > 255 ? 255 :
                                  ss->sector->lightlevel) * (1.0f / 255.0f);
        ...
        for (const mobj_t *mo = ss->sector->thinglist; mo; mo = mo->snext) {
            ...
            { extern int r_ss_emitted; r_ss_emitted = 1; }
#endif
            r_sprite_add(cur_cam, &t,
                         (mo->frame & FF_FULLBRIGHT) ? 1.0f : lit);
        }
    }
    ...
    for (int i = 0; i < ss->numlines; i++) {
        ...
        continue;
#endif
        /* P_LoadSegs already resolved which side of the linedef this seg
         * faces and the sectors either side of it, so the manual sidenum
         * lookup this used to do is gone along with its bounds checks. */
        const side_t   *fs    = sg->sidedef;
        const sector_t *front = sg->frontsector;
        if (!fs || !front) continue;

        const vertex_t *a = sg->v1;
        const vertex_t *b = sg->v2;

        const float ax = fx(a->x), ay = fx(a->y);
        const float bx = fx(b->x), by = fx(b->y);
        ...
        const sector_t *back = sg->backsector;   /* NULL on a one-sided line */
         ...
         * before being noticed. Four loads and compares dispose of them
         * before any of that. The degenerate same-AND-closed case (zero
         * height both sides) still falls through to the closed-door logic
         * below, which is where it went before. */
        if (back) {
            const float bf = fx(back->floorheight);
            const float bc = fx(back->ceilingheight);
            /* A masked mid keeps an identical-heights line alive: a fence
             * strung across one sector is exactly such a line, and it still
             * has something to draw even though it clips nothing. */
            if (bc == fceil && bf == ffloor && fs->midtexture == 0 &&
                !(bc <= ffloor || bf >= fceil)) { stat_same++; continue; }
        }

        int x1, x2;
        float xf[8];
        if (!screen_span(ax, ay, bx, by, &x1, &x2, xf)) { stat_span++; continue; }
         ...
         * line stayed unmapped however far the player walked: the automap
         * drew the player arrow and the crosshair and nothing else. Here is
         * the same moment -- the seg has a real span on screen. */
        ...
        /* Horizontal alignment: the sidedef's offset plus how far along the
         * linedef this seg starts. Without the seg offset, every seg cut from
         * one wall restarts the texture at zero and the wall visibly steps.
         * Length and the endpoint transform both come precomputed: the
         * length from load time (static geometry), the transform from
         * screen_span a few lines up. */
        r_wall_t w = {
            .x1 = ax, .y1 = ay, .x2 = bx, .y2 = by,
            .u0 = fx(fs->textureoffset) + (float)sg->offset,
            .len = r_seglen[sg - segs],
            .light = light,
            .cd1 = xf[0], .co1 = xf[1], .cd2 = xf[2], .co2 = xf[3],
            .has_xform = 1,
        };
        ...
        if (!back) {
            ...
            w.tex = p_level_texture(fs->midtexture);
            if (w.tex) {
                const float texh = (float)w.tex->height;
                w.zbot = ffloor; w.ztop = fceil;
                w.ztex = ((ld->flags & ML_DONTPEGBOTTOM) ? ffloor + texh : fceil)
                         + rowoff;
                if (w.ztop > w.zbot) {
                    cur_wall = w;
        ...
        const float bfloor = fx(back->floorheight);
        const float bceil  = fx(back->ceilingheight);
        ...
        if (bceil <= ffloor || bfloor >= fceil) {
             ...
             * fence invisible. */
            if (fceil > bceil) {
                w.tex = p_level_texture(fs->toptexture);
                if (w.tex) {
                    const float texh = (float)w.tex->height;
                    w.zbot = bceil; w.ztop = fceil;
                    w.ztex = ((ld->flags & ML_DONTPEGTOP) ? fceil : bceil + texh)
                             + rowoff;
                    cur_wall = w;
                    clip_solid_close(x1, x2);
                    continue;
            ...
            if (bfloor > ffloor) {
                w.tex = p_level_texture(fs->bottomtexture);
                if (w.tex) {
                    w.zbot = ffloor; w.ztop = bfloor;
                    w.ztex = ((ld->flags & ML_DONTPEGBOTTOM) ? fceil : bfloor)
                             + rowoff;
                    cur_wall = w;
                    clip_solid_close(x1, x2);
                    continue;
         ...
         * by the depth-tested sky pass, whose span they joined above. */
        {
            ...
            const float fda = cur_cam->focal / xf[4];
            const float fdb = cur_cam->focal / xf[5];
            clip_narrow(x1, x2,
...
static bool child_visible_inner(int nodenum, int child)
{
     ...
     * for the one toward it. */
    {
        ...
        if (wtop > 0.0f || wbot < (float)SCREEN_H) {
            const float px = cur_cam->x, py = cur_cam->y;
            const float txl = (nf->xl[child] - px) * view_cs;
            const float txh = (nf->xh[child] - px) * view_cs;
            const float tyl = (nf->yl[child] - py) * view_sn;
            const float tyh = (nf->yh[child] - py) * view_sn;
            float dmin = fminf(txl, txh) + fminf(tyl, tyh);
            float dmax = fmaxf(txl, txh) + fmaxf(tyl, tyh);
            if (dmin < BSP_NEAR) dmin = BSP_NEAR;
            if (dmax < dmin)     dmax = dmin;

            const float cyf = SCREEN_H * 0.5f;
            const float f   = cur_cam->focal;
...
static void render_node(int num)
{
    while (!(num & NF_SUBSECTOR)) {
        ...
        if (child_visible(num, side)) render_node(n->children[side]);
        else                          stat_culled++;

        if (!child_visible(num, side ^ 1)) { stat_culled++; return; }
        num = n->children[side ^ 1];
    }
...
void r_render_level(const r_camera_t *cam)
{
    ...
    stat_em_solid = stat_em_pass = in_pass = 0;

    r_pvs_frame(cam->x, cam->y);
    r_set_view(cam);                       /* fills r_view_cs / r_view_sn */
    view_k = (SCREEN_W * 0.5f) / cam->focal;
    r_flat_stamp++;                        /* invalidate region stamps */
    clip_reset();

    /* Doors and lifts move sector heights, and the vertical cull reads
     * per-subtree height ranges derived from them. One pass of integer adds
     * notices a change; the ranges rebuild only on the frames it happens. */
    {
        static int64_t last_hsum = -1;
        int64_t hsum = 0;
        for (int i = 0; i < numsectors; i++)
            hsum += (int64_t)sectors[i].floorheight * 3 +
                    (int64_t)sectors[i].ceilingheight;
        if (hsum != last_hsum) { last_hsum = hsum; R_RefreshNodeZ(); }
    }
