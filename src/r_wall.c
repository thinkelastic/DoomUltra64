...
static r_quad_t          quads[R_MAX_QUADS];
 ...
 * 1 at FOG_NEAR, falling to 0 at FOG_FAR. Multiplied into shade RGB. */
                     ...
                     float xmin, float xmax, float ymin, float ymax)
{
    ...
    const float kl = (xmin - cxm) / cam->focal;   /* left  bound  */
    const float kr = (xmax - cxm) / cam->focal;   /* right bound  */
                      ...
                      float ymin, float ymax)
{
    ...
    float uscale = 1.0f;
    {
        ...
        while (tex->mip && dnear > cam->focal / uscale) {
            tex = tex->mip;
            uscale *= 0.5f;
        }
    }
...
#define R_COLUMNS 0
     ...
     * tiled quad path, which still works. */
    {
        ...
        if (ct->width * ct->height <= 2048 && num_spans < R_MAX_SPANS) {
            ...
            if (da > 0.0f && db > 0.0f) {
                ...
                const float xa = cx + oa * cam->focal * iza;
                const float xb = cx + ob * cam->focal * izb;
    ...
    if (s_ok && t_ok && texw * texh <= 2048) {
        ...
        if (da > 0.0f && db > 0.0f) {
            ...
            const float xa = cx + oa * cam->focal * iwa;
            const float xb = cx + ob * cam->focal * iwb;

            const float sha = light * visibility_at(da);
            const float shb = light * visibility_at(db);

            const float t_lo = (wall->ztex - wall->ztop) * uscale;
            const float t_hi = (wall->ztex - wall->zbot) * uscale;

            float ya_t = cy - (wall->ztop - cam->z) * cam->focal * iwa;
            float ya_b = cy - (wall->zbot - cam->z) * cam->focal * iwa;
            float yb_t = cy - (wall->ztop - cam->z) * cam->focal * iwb;
            float yb_b = cy - (wall->zbot - cam->z) * cam->focal * iwb;
            ...
            if (!((ya_b <= ymin && yb_b <= ymin) ||
                ...
                if (q) {
                    ...
                    set_vtx(q->v[0], xa, ya_t, sha, sa, ta_t, iwa);
                    set_vtx(q->v[1], xb, yb_t, shb, sb, tb_t, iwb);
                    set_vtx(q->v[2], xb, yb_b, shb, sb, tb_b, iwb);
                    set_vtx(q->v[3], xa, ya_b, sha, sa, ta_b, iwa);
                }
            }
        }
        return;
    ...
    wctx_t c = {
        ...
        .uscale     = uscale,
        .inv_uscale = 1.0f / uscale,     /* exact: uscale is a power of two */
        ...
        .win_lo = ymin,
        .win_hi = ymax,
        .vrep_first = (int)floorf(t_lo / (float)texh),
        .tile_h = tile_h,
        .texh   = texh,
    ...
    if (!wall->masked && (tile_w == texw) && pot_w && (sb - sa) < 1023.0f) {
        const float rep_base = floorf(sa / (float)texw) * (float)texw;
    ...
    const int rep_first = (int)floorf(sa / (float)texw);
                     ...
                     int ts0, int ts1)
{
    ...
    const float shb = c->light * visibility_at(db);
    ...
    for (int vrep = c->vrep_first; ; vrep++) {
        ...
        for (int src_t = 0; src_t < texh; src_t += tile_h) {
            ...
            float ya_t = cy - (ztop - cam->z) * cam->focal * iwa;
            float ya_b = cy - (zbot - cam->z) * cam->focal * iwa;
            float yb_t = cy - (ztop - cam->z) * cam->focal * iwb;
            float yb_b = cy - (zbot - cam->z) * cam->focal * iwb;
            ...
            set_vtx(q->v[0], xa, ya_t, sha, s_a, ta_t, iwa);  /* top-left  */
            set_vtx(q->v[1], xb, yb_t, shb, s_b, tb_t, iwb);  /* top-right */
            set_vtx(q->v[2], xb, yb_b, shb, s_b, tb_b, iwb);  /* bot-right */
            set_vtx(q->v[3], xa, ya_b, sha, s_a, ta_b, iwa);  /* bot-left  */
...
static void emit_span_columns(const r_span_t *sp)
{
    ...
    for (int x = x0; x < x1; x++) {
        ...
        const float scale = cam_focal * iz;
...
void r_flush_walls(void)
{
    ...
    /* Group quads by texture first, so the per-tile scan only looks at quads
     * that could match.
     *
     * The naive form -- for every texture, for every tile, scan every quad --
     * is O(textures x tiles x quads), which at fifteen textures and a few
     * hundred quads is tens of thousands of iterations a frame purely to
     * decide what to skip. A counting pass over the texture index collapses it
     * to O(quads x tiles): same uploads, same drawing, an order of magnitude
     * less bookkeeping. */
    uint16_t start[R_MAX_TEXTURES + 1] = {0};
    static uint16_t order[R_MAX_QUADS];

    for (int k = 0; k < num_quads; k++) start[quads[k].texidx + 1]++;
    for (int i = 0; i < num_batch_tex; i++) start[i + 1] += start[i];

    {
        uint16_t cursor[R_MAX_TEXTURES];
        for (int i = 0; i < num_batch_tex; i++) cursor[i] = start[i];
        for (int k = 0; k < num_quads; k++) order[cursor[quads[k].texidx]++] = (uint16_t)k;
    }

    for (int i = 0; i < num_batch_tex; i++) {
        const dt64_tex_t *tex = batch_tex[i];
        const int lo = start[i], hi = start[i + 1];
        if (lo == hi) continue;

        const int tile_w = tex->width  < DT64_TILE_W ? tex->width  : DT64_TILE_W;
        const int tile_h = tex->height < DT64_TILE_H ? tex->height : DT64_TILE_H;

        /* Band wrapping is a property of the texture's shape, not of any one
         * quad: when the tile spans the full (power-of-two) texture along an
         * axis, the emitter merges repeats into single wrapped quads along
         * it, and every upload of such a texture wraps that axis. Quads that
         * happen to stay inside one repeat sample identically either way. */
        const bool band_s = (tile_w == tex->width) &&
                            (tex->width  & (tex->width  - 1)) == 0;
        const bool band_t = (tile_h == tex->height) &&
                            (tex->height & (tex->height - 1)) == 0;

        for (int t0 = 0; t0 < tex->height; t0 += tile_h) {
            for (int s0 = 0; s0 < tex->width; s0 += tile_w) {
                bool resident = false;

                for (int n = lo; n < hi; n++) {
                    const r_quad_t *q = &quads[order[n]];
                    if (q->s0 != s0 || q->t0 != t0) continue;

                    if (!resident) {
                        if (q->wrap) {
                            /* Only the axes that are power-of-two may repeat;
                             * the others clamp, which is correct because the
                             * fast path is only taken when the wall stays
                             * inside the texture along them. */
                            rdpq_texparms_t p = {0};
