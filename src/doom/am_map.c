...
/* Port: no CPU frame buffer; lines go through r_am_line. */
static int 	amclock;
...
#if D_LEVELTEST
    cheating = 1;      /* flow driver never explores; show every line */
#endif
...
{
    /* Port: the frame is cleared black at attach and the 3D view is not
     * drawn while the map is up, so the background is already there. */
    (void)color;
}
...
( fline_t*	fl,
  int		color )
{
    /* Port: the Bresenham plotter becomes one RDP thin-quad per line; the
     * clipping above already bounded the endpoints to the frame. */
    ...
    r_am_line(fl->a.x, fl->a.y, fl->b.x, fl->b.y, color);
}
...
{
    void r_am_line(int x0, int y0, int x1, int y1, int palette_index);
    const int cx = f_w / 2, cy = f_h / 2;
    r_am_line(cx - 2, cy, cx + 2, cy, color);
    r_am_line(cx, cy - 2, cx, cy + 2, color);
}
