...
void F_TextWrite (void)
{
    ...
    // erase the entire screen to a tiled background. The port draws it
    // through the RDP from the baked flat rather than into a CPU buffer.
    {
        void V_DrawFlatFill(const char *flatname);
        V_DrawFlatFill(finaleflat);
    }
...
void F_CastDrawer (void)
{
    ...
    if (flip)
	/* Port: no flipped blit in the baked-texture path; the cast call
	 * is Doom II only and this build is shareware. */
...
void F_BunnyScroll (void)
{
    ...
    /* Port: the per-column software blit becomes two offset patch draws --
     * the same pixels, but through the RDP. p1 shows from -scrolled, p2
     * continues to its left; the scissor clips whatever runs off screen. */
    (void)x;
    V_DrawPatch(-scrolled, 0, p1);
    if (scrolled > 0)
        V_DrawPatch(SCREENWIDTH - scrolled, 0, p2);
