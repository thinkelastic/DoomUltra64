...
#define SAVE_PREFIX_MAX 48
static char save_prefix[SAVE_PREFIX_MAX] = "doom";
static char save_filenames[10][64];
...
void P_SetSavePrefix(const char *prefix)
{
    ...
    while (base[len] != '\0'
        && base[len] != '.'
        && len < sizeof(save_prefix) - 1)
    {
        save_prefix[len] = base[len];
        ++len;
    }
    ...
    save_prefix[len] = '\0';
    memset(save_filenames, 0, sizeof(save_filenames));
}
...
char *P_SaveGameFile(int slot)
{
    ...
    if (save_filenames[slot][0] == '\0')
    {
        DEH_snprintf(save_filenames[slot],
                     sizeof(save_filenames[slot]),
                     "%s_%d.sav", save_prefix, slot);
    }

    return save_filenames[slot];
}
