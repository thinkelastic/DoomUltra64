...
menuitem_t MainMenu[]=
{
    ...
    // Quit Game is gone. A cartridge has nothing to quit to: the entry led
    // to I_Quit, which on this console is an assertion screen and a reset.
    // M_QuitDOOM itself stays, since the F10 binding still reaches it.
};

menu_t  MainDef =
{
    // One item shorter than main_end, Quit having been removed; y moves down
    // half a line to keep the block centred where it was.
    main_end - 1,
    NULL,
    MainMenu,
    M_DrawMainMenu,
...
menu_t  LoadDef =
{
    load_end,   /* saves go to the card now; see i_n64.c */
    &MainDef,
    LoadMenu,
    M_DrawLoad,
    80,54,
...
void M_ReadSaveStrings(void)
{
    ...
    for (i = 0;i < load_end;i++)
    {
        ...
        if (!I_SaveSlotReadHeader(P_SaveGameFile(i),
                                      header, sizeof(header))
         || !M_SaveGameHeaderValid(header))
        {
...
{
    // Name the slot after where the player is -- "E1M1: HANGAR" -- the same
    // rule the reference tree uses, so a console save needs no typing at all.
...
void M_SaveSelect(int choice)
{
    ...
    /* Port: there is no keyboard, so every save behaves as joypad-initiated:
     * the name below defaults to the current level and A confirms it. */
    joypadSave = true;
...
void M_Init (void)
{
    ...
    if (gamemode == commercial)
    {
        MainDef.numitems--;      /* no Read This in Doom II */
        MainDef.y += 8;
