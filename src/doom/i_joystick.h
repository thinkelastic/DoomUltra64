...
/* SDL include removed: the controller is read through libdragon's
 * joypad API in main.c and turned into a ticcmd directly, so no
 * backend is needed here -- only the direction encoding below.
 *
 * SDL_CONTROLLER_BUTTON_MAX survives as an array bound in the binding table.
 * It only has to be large enough for the buttons the config code can name, and
 * nothing on this target reads that table -- main.c maps the pad itself. */
