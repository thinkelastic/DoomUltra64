...
void P_InitSwitchList(void)
{
	...
	{
	    /* Port: register a pair only when the level already references one
	     * of its two textures. R_TextureNumForName here does not just name
	     * a texture, it LOADS it, and walking the whole switch list would
	     * drag eighty wall textures into the arena for the handful of
	     * switches a map has. Either side counts, since a map may start a
	     * switch already thrown. */
	    int p_level_lookup(const char *name, const char *prefix);
	    if (p_level_lookup(DEH_String(alphSwitchList[i].name1), "") < 0 &&
	        p_level_lookup(DEH_String(alphSwitchList[i].name2), "") < 0)
		continue;
...
//  R_PrecacheLevel marks only the textures on sidedefs, i.e. each switch's
//  CURRENT state.  P_ChangeSwitchTexture swaps a pressed switch to its paired
//  texture, which is on no sidedef and so would never get a GPU 2D block --
//  drawing it (a screen-filling wall when you stand right in front) would fall
//  back to a slow CPU column draw.  Expand the present set so both states of any
//  switch used on the level are precached (mirrors the animated-texture
//  expansion).  Cheap: switch textures are small and few.
//
